...
| 4.2a FAB drawer + Sync runs with a spinner | REMOVED by Epic 11 story 11.1; the now-empty one-action FAB shell was subsequently deleted. Absence across Shelf, Catalog, and Stats is pinned by `epic4-settings.spec.ts` › no destination renders a Chores FAB |
...
| 5.1b FAB "Check PS+ Extra" runs the check with a spinner | REMOVED by Story 8.4 — refreshes are automatic; no manual check control or FAB remains |
...
| 5.1f summary modal reports the flag changes | REMOVED with the manual refresh action by Story 8.4; scheduled refresh state surfaces through the header readout and persistent banners |
...
| 6.3a Export CSV → full library downloads from D1 | `epic6.spec.ts` › Export CSV: Settings downloads … (download event + filename); success/progress/failure-without-save pinned in `SettingsPanel.test.tsx`; CSV content pinned in `export.test.ts` + `csv.test.ts` |
| 6.3b FAB handedness moves the button and persists | RETIRED — FAB and placement control/API are deleted; `settings.test.ts` pins that a legacy row is ignored and the removed endpoint returns 404 |
...
| 6.4b every manual own prompts buy-vs-claim (gate dropped 2026-07-23 — sync gone, Essential titles are outside the Extra catalog); a claim writes owned_via=membership without bought_on | `epic6.spec.ts` › owning a non-PS+ game prompts too; "Claimed with PS+" writes owned_via=membership, no bought_on; the ungated prompt also jsdom-pinned in `Card.test.tsx` + `DetailPanel.test.tsx`; default-to-purchase still pinned in `tracking.test.ts` › defaults owned_via=purchase when no source is sent |
| 6.4b detail "Claimed with PS+" corrects a purchase (or legacy NULL) to a claim; bought_on preserved (write-once) | `epic6.spec.ts` › detail "Claimed with PS+" corrects a purchase to a claim, preserving bought_on (hazard: the stamped date survives the downgrade); button render/absence + via=membership PATCH + toast jsdom-pinned in `DetailPanel.test.tsx` |
| 6.4b add-by-name owned adds carry a source: inline Purchased/Claimed radios (default Purchased); a claimed add writes owned_via=membership with NO bought_on; catalog adds still refuse owned regardless of via | `epic6.spec.ts` › add-by-name as "Claimed with PS+" writes owned_via=membership, no bought_on; radios render/absence + payload via jsdom-pinned in `AddGameDialog.test.tsx` (owned adds carry a source describe); membership branch, explicit-purchase twin, via-can't-bypass-H1 refine, AND via-without-owned 400 all pinned server-side in `games.test.ts` |
...
| 9.2 FAB trigger + summary readout | REMOVED by Epic 11 story 11.1 — the trophy item, mutation, and `TrophySyncModal` are deleted; the later FAB retirement is pinned by `epic4-settings.spec.ts` |
...
| AC | Coverage |
|----|----------|
| 9.5a a second PSN op is refused with a 409 + a human message; no PSN call is made; another user is never blocked | the three credentialed routes are REMOVED by Epic 11 story 11.1 (and story 11.2 trimmed `PsnOp` to `catalog-refresh` alone); the surviving lock is pinned in `psn-lock.test.ts` › two concurrent claims: exactly ONE wins + › is per USER + › refuses a second catalog op with a 409 + › a CURSOR is not a capability |
...
(`epic1-shelf`, `epic2-detail`, `epic7-catalog` — shelf, by-id detail, catalog
paging/markers all drive the changed paths).

| AC | Coverage |
|----|----------|
| 8.6a `GET /games/:id` is a single-row read, payload unchanged | no NEW UI flow — `epic2-detail.spec.ts` + `epic7-catalog.spec.ts` deep-link specs drive the route; parity + scope-miss pinned in integration `read-budget.test.ts` › 8.6a (card identical to the whole-library bake; unknown/other-user/discarded all null) |
| 8.6b settings counts via SQL `COUNT(*)` | no UI flow (numbers render as before — `SettingsPanel.test.tsx` unchanged) — integration `read-budget.test.ts` › 8.6b (full-scan truth incl. zero and discarded exclusion) |
| 8.6c shelf ETag/304 + the version-bump invariant | no UI flow (a 304 renders identically by definition) — integration `read-budget.test.ts` › 8.6c: the route 304s on an unchanged version, answers 200 + a NEW tag after a write, and EVERY writer category rotates the version (the bypass suite); the client 304 path is `callApi`'s retained-body branch, exercised by the route tests' contract |
| 8.6d catalog SQL paging + page-scoped markers | `epic7-catalog.spec.ts` re-run green over the paged reads (grid, filters, markers, add-flow); page-union/order/filter + non-zero-page markers pinned in integration `read-budget.test.ts` › 8.6d |
| 8.6e `session.cookieCache` enabled, TTL ≤ 5 min | no UI flow — config pinned in `src/services/auth.ts` (`maxAge: 300`); the auth journey (`auth-journey.spec.ts`) re-runs green with the cache on |
...
| 7.2c genre filter narrows the grid on the PS-store facet KEYS (never IGDB genres), using one desktop menu or one phone Filters bottom sheet | `epic7-catalog.spec.ts` › the genre filter narrows the grid (PS-store facet keys, OR within the group) — desktop keyboard-opens the Genre menu and asserts the URL carries `genre=HORROR`, the key, never the rendered label; › phone catalog hides inline genres and filters live in a bottom sheet — 375×667 pins single-trigger placement, modal semantics, live URL/grid update, settled count, active badge, CTA focus return, and real-CSS breakpoint dismissal/focus handoff to desktop Genre. jsdom `Catalog.test.tsx` pins pending-count honesty, late vocabulary focus, duplicate-key collapse, modal focus/Escape/backdrop/scroll/breakpoint behavior, orphan/failure escape paths, and `genreLabel` |
...
| 7.2l a same-title CUSA/PPSA edition pair with OVERLAPPING platform lists collapses onto one card (the store often marks both SKUs `["PS4","PS5"]` — probed 2026-07-23) | `epic7-catalog.spec.ts` › a same-title CUSA/PPSA pair with overlapping platforms renders ONE card — the PPSA SKU. The full predicate matrix (shared np_title_id, cross-gen prefix, legacy disjointness, both-PPSA/null-id-overlap stay two cards, facet parity) in integration `psplus-browse.test.ts`; the bare trailing platform-suffix strip in `core/title-normalizer.test.ts` |
...
| AC | Coverage |
|----|----------|
| 11.1a the FAB drawer offers only surviving non-credentialed actions | SUPERSEDED — manual PS+ refresh was removed, export moved to Settings, and the empty FAB shell was deleted. `epic4-settings.spec.ts` pins no FAB on any destination and the Settings backup action |
...
| 11.1c the anonymous PS+ catalog check, monthly cron, and CSV export pass unmodified | provider/export behavior stays pinned by `epic5-psplus.spec.ts`, `epic7-catalog.spec.ts`, `psplus*.test.ts`, `export.test.ts`, and `epic6.spec.ts` › Export CSV from Settings |
