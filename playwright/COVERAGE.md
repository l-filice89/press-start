...
| 9.3 Settings trigger, cursor loop, and the end-of-run summary — every ending DISTINCT | `epic4-settings.spec.ts` › Settings carries the platinum-date backfill, and a run with no trophy data says to sync trophies first (the zero-candidate path, the only PSN-free one — it MOVED to that serial file in Story 9.5: the backfill now takes the per-user single-flight lock, and the e2e suite's one user cannot click it in a parallel worker while a FAB sync runs); jsdom `SettingsPanel.test.tsx` › LOOPS the backfill on its cursor… + › says so when there is nothing to recover + › with NO trophy data at all, points at the trophy sync + › an ALL-SKIP run says PlayStation had no record for any of them + › a rejected token stops the loop (and refetches settings so the banner lights) + › a token that expires MID-RUN still reports what was already recovered + › says the run STOPPED EARLY when the chunk brake trips |
...
| AC | Coverage |
|----|----------|
| 9.5a a second PSN op (library sync / trophy sync / platinum backfill) is refused with a 409 + a human message; no PSN call is made; another user is never blocked | skipped e2e — driving it needs two CONCURRENT real PSN runs, which the e2e Worker cannot stub; pinned against real workerd + D1 in `psn-lock.test.ts` › two concurrent claims: exactly ONE wins (the atomicity hazard) + › refuses a second PSN op with a 409 and a message — and makes NO PSN call (all three routes) + › is per USER + **› a CURSOR is not a capability: a backfill continuation cannot steal a running sync's lock** (the review found the first cut honoured any `?cursor=`, which made the refusal bypassable — the lock now keys on a token, and a forged/stale one is refused) + › a continuation that presents its OWN token renews the lock and proceeds (and the token ROTATES, so a replayed chunk cannot renew); the UI half in jsdom `Fab.test.tsx` + `SettingsPanel.test.tsx` › surfaces the single-flight refusal — not a generic failure |
| 9.5b the lock is RELEASED on every exit (success or failure) and expires if a run dies | skipped e2e — same unstubbable-PSN constraint; `psn-lock.test.ts` › releases on the way out + › releases even when the run FAILS (a 502 must not lock the user out) + › expires: a lock left behind by a crashed run is taken over after its TTL + › a release only ever clears the caller's OWN lock |
...
| 9.5d a DISCARDED game's trophy title is matched and dropped SILENTLY — neither updated nor "unmatched" noise on every run | skipped e2e — the trophy sync run is unstubbable (9.2a); `trophies.test.ts` › drops a DISCARDED game's trophy title SILENTLY (the tombstone's trophy columns asserted byte-equal after the run) |
...
| 7.3j the catalog `np_title_id` is anchored so a later library sync matches instead of duplicating | **NOT e2e-able**: the sync needs a live PSN response. Integration `games.test.ts` › HAZARD: the catalog np_title_id is anchored as PSN, so a later sync MATCHES a diverged title (runs the real `planSync` over the real link rows) |
