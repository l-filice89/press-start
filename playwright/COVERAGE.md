...
| 7.3j the catalog `np_title_id` is anchored so a later library sync matches instead of duplicating | **NOT e2e-able**: the sync needs a live PSN response. Integration `games.test.ts` › HAZARD: the catalog np_title_id is anchored as PSN, so a later sync MATCHES a diverged title (runs the real `planSync` over the real link rows). Epic 11 story 11.1 removed library sync, so no production flow reaches `planSync` anymore — the row pins the anchoring invariant at the domain seam only |
