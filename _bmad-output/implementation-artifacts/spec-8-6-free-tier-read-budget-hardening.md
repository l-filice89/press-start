...
2026-07-17 — independent pass RAN (fresh reviewer, no shared context) over 14ae6d2..HEAD, auditing EVERY shelf-DTO writer beyond the two patched seams. Both HIGH patches HOLD (`resolveStraggler` both paths, `addGame` shared-row branches, `rematchGame`; fresh-create actor-only correct). **One NEW HIGH in the same class, FIXED**: genre add/remove writes the shared `game_genre` table (game-scoped, no userId — both reads pull genres per game) but bumped only the actor, leaving every co-tracker on a permanent stale 304; both writers now `bumpAllLibraryVersions`, pinned by a co-tracker assertion driven through the real service seam. **MED FIXED**: seed-import's existing-row PSN link recovery flips the derivation's np-leg for co-trackers with only an actor bump → bump-all when a shared link was recovered. **MED FIXED**: migration 0018 rewrote shelf-rendered `owned_via` with no rotation → migration 0019 rotates every `library_version` row (one UPDATE). Seam finding on the cron (rename-only snapshot: `moved` compares product-id sets while the upsert refreshes the derivation's join keys) → membership pass now bumps UNCONDITIONALLY on success. LOW noted, not changed: naive comma-split of `If-None-Match` (our tags carry no commas; mis-split costs a spurious 200 — safe direction). LOW fixed: `psplus-browse` passed a userId to the region-keyed sweep-state read (silent `string`-typed miss). Confirmed clean: all remaining writer categories, ETag race directions (every race degrades to a spurious 200, never a stale 304), `?include=hidden` per-URL caching, by-id DTO parity by shared selection, catalog paging total order + generation re-key, cookieCache bound exactly the accepted AD-33 §6 revocation latency. Obligation discharged; merge gate clear for this story.
