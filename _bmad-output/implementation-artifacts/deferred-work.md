...
- source_spec: `_bmad-output/implementation-artifacts/spec-2-1-change-play-status-from-the-shelf.md`
  ...
  resolution: resolved 2026-07-10 by story 3.4 (per its own decision line — focus restoration to clamped-index neighbor, UNDO Tab-reachable); resolution field backfilled 2026-07-24 ledger groom.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-2-4-edit-ownership-and-lifecycle-dates-in-detail.md`
  ...
  resolution: resolved 2026-07-10 by story 3.4 (ref-backed guardPending shared by all entry points incl. UNDO closures); resolution field backfilled 2026-07-24 ledger groom.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-platinum-only-auto-hide.md`
  ...
  resolution: resolved 2026-07-10 by story 3.2 (visibility compared before/after per its decision line); resolution field backfilled 2026-07-24 ledger groom.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-2-5-3-backfill-epic-2-e2e-flows.md`
  ...
  resolution: resolved 2026-07-10 by story 3.4 (openGameId hoisted to ShelfGrid); resolution field backfilled 2026-07-24 ledger groom.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-5-1-region-setting-ps-extra-check-button.md`
  ...
  resolution: resolved 2026-07-22 by story 8.3 (spec-8-3-per-user-ps-facts-region-and-catalog-flag-b2-b3, status done, shipped v3.0.0) — the global column is gone from the schema; verified by code probe 2026-07-24.
- source_spec: `_bmad-output/implementation-artifacts/spec-5-1-region-setting-ps-extra-check-button.md`
...
- source_spec: `src/services/settings.ts` `getPsnRegion` (Luca 2026-07-11)
  ...
  resolution: resolved 2026-07-22 by story 8.3 (B3 — per-user `psn_region` SETTING + editor, shipped v3.0.0); the entry's own close condition ('ledger closes when 8.3 ships') fired. Verified 2026-07-24.
...
- source_spec: `spec-6-4-ownership-source-purchased-vs-claimed.md`
  ...
  resolution: resolved 2026-07-22 by story 8.3 (B2 — all three flag writers migrated per-user, shipped v3.0.0); `setPsPlusExtraFlags` global write gone. Verified 2026-07-24.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-sign-in-with-google-b1a.md`
  ...
  resolution: closed 2026-07-17 (Story 8.2) — MOOT under the signed-off AD-29: registration is open, the allowlist (and with it "de-allowlisted") no longer exists, and no de-admission concept replaces it (no ban tooling until real abuse appears). A valid session alone gates; revocation latency is bounded by the ≤5-min cookieCache TTL (AD-33 §6).
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-sign-in-with-google-b1a.md`
  ...
  resolution: closed 2026-07-17 (Story 8.2) — two halves per AD-29: (1) rate limiting is a Cloudflare WAF rule on `/api/auth/*` (edge, free tier includes one; deploy step recorded in spec-8-2's Verification — never D1 counters, which would hand an attacker a write per anonymous hit); (2) the residue itself is swept — `deleteExpiredVerifications` runs on cron sweep invocations (worker/index.ts), proven by an integration test that expires a stranger's row and sweeps it while a live one survives.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-sign-in-with-google-b1a.md`
  ...
  resolution: closed 2026-07-17 (Story 8.2) — `account.accountLinking.trustedProviders` is now explicitly EMPTY (`src/services/auth.ts`): a trusted provider would link even when the provider reports the email UNVERIFIED, and with open registration anyone can pre-register a victim's address. Linking therefore demands the provider-verified matching email. The bypass is driven in `auth.test.ts` through better-auth's own `handleOAuthUserInfo` (the exact callback seam): unverified match refused with no account row; verified match links.
...
resolution: done 2026-07-16 (Story 10.2) — DECIDED: `first_seen_at` keeps its "first seen since the last prune" meaning, documented at the `upsertCatalogProducts` header (src/repositories/psplus-catalog.ts). The 10.2 warning deliberately does NOT read it: departure derives from the game-level flag transition (`toClear` stamps `game.ps_plus_left_on`, `toFlag` NULLs it), so a pruned-then-readded title re-enters via `toFlag` and can never misread as a new arrival. Hazard pinned red-then-green in test/integration/psplus-departure.test.ts ("DW-13 HAZARD" case). No rename — nothing reads the column; any future catalog-history consumer is warned at the write site.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-10-1-critic-user-scores-on-every-game-vr-5.md`
  summary: The failed-score-refresh banner has no user-triggerable retry — a failure on the cron window's last day (21st) leaves the banner lit ~3 weeks until day 15 of the next month.
  evidence: Unlike the PS+ banner ("run Check PS+ Extra from the menu"), no manual refresh path exists for scores; the banner copy honestly says "retries on the next scheduled run", but that run can be weeks away. Candidate fix: a Settings/FAB "refresh scores now" action reusing runScoreRefresh, or an explicit acceptance note. Deliberate 10.1 scope cut, single-user tolerable.
  resolution: done 2026-07-19 (final deferred-work sweep, Luca's call: more frequent cron, not a button) — the score refresh rides its OWN daily trigger (`0 3 * * *`, wrangler.jsonc; worker/index.ts routes by `controller.cron`) instead of piggybacking the monthly PS+ window. The 7-day stale-gate makes most fires a no-op read, and a FAILED run never stamps, so it retries the next morning — worst case for a lit banner drops from ~3 weeks to ~1 day. Side benefit: scores never share an invocation with the PS+ work, so the ~49/50 combined subrequest worst case is gone.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-10-3-time-to-beat-the-story-and-100-vr-8.md` (found by the 2026-07-16 independent follow-up review)
  ...
  resolution: DISCARDED 2026-07-16 (Epic 10 retro, Luca's call) — the FR-40 banner already signals stragglers; every straggler in prod is already fixed (rematch-anchored); and if the app ever goes public new users start from a blank catalog, never the seed path. Won't fix.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-10-5-scores-in-add-modal-color-graded.md`
  ...
  resolution: done 2026-07-19 (final deferred-work sweep) — `.sr-only` moved from `web/shelf/card.css` to the global base sheet `web/index.css`, where every consumer reaches it regardless of bundling.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-minor-ux-bugs-sweep.md`
  ...
  resolution: done 2026-07-19 (final deferred-work sweep) — `listCatalogGenreFacets` returns the tagged keys directly; the sweep-state read, the vocabulary union, AND the now-equally-dead zero-count filter (every remaining key has ≥1 card by construction) are deleted. The zero-count contract stays pinned by the existing "omits a zero-count key even when the sweep vocabulary names it" test.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-3-per-user-ps-facts-region-and-catalog-flag-b2-b3.md` (epic-8 follow-up review, 2026-07-17, MED)
  summary: The leaving sweep targets catalog products by normalized TITLE only, while the 8.3 membership derivation grants membership through three legs — a game added from the catalog (PSN_PRODUCT-linked, IGDB-retitled) shows the PS+ pill via the link leg but is silently skipped by the sweep, so its leaving date can never populate.
  evidence: `src/services/psplus-leaving.ts:92-141` (`products.get(normalizeTitle(row.title))`; `if (!product) continue`) vs `src/repositories/games.ts` `librarySelection` (three-key EXISTS). `games.ts:127-129` documents that the add path re-seeds the title from the IGDB candidate, so title drift is routine for exactly the games the catalog add creates. Fix is the sweep resolving targets through the same three keys (join tracked games via `external_link` to catalog products), not a bigger title map — a sweep-shape change, not a patch.
  decision: 2026-07-17 deferred from the epic-8 follow-up review — needs the sweep's target-selection redesigned against the 8.3 keys; too large for a review patch. Candidate first story of the next PS+ touch.
  resolution: done 2026-07-19 (final deferred-work sweep, Luca's call: fix now) — the sweep resolves each game's catalog product through the SAME three legs as the 8.3 derivation, in the same priority: PSN_PRODUCT link > PSN np-title link > recomputed normalized title. `listRegionTrackedGames` now carries the games' PSN link ids (one LEFT JOIN, still one D1 call — budget unchanged), and `runLeavingSweep`'s `resolveProduct` walks the legs with sorted ids/products so a multi-link or duplicate-key case resolves deterministically. Link-resolved ledger rows carry the CATALOG's normalized title (the row describes the product; the game's title is drifted by definition). Pinned by "a catalog-added game (PSN_PRODUCT link, IGDB-retitled) gets its date through the LINK leg" in test/integration/psplus-leaving.test.ts.

- source_spec: `_bmad-output/implementation-artifacts/spec-8-4-the-scheduled-refresh-serves-every-region-b4.md` (epic-8 seam review, 2026-07-17, MED — orphaned delegation)
  summary: `runScheduledScoreRefresh` still resolves `findOldestUser`, so `scores_refresh_failed`/`scores_refreshed_at` are written only for user #1 — every other user's FR-40 failure banner can never light; score failures are silent staleness for them. The code marker "8.4 owns the real model" points at a COMPLETED story that scoped it out, so no open story carries the work (DELEGATED-WORK-CARRIES-AN-AC violation).
  evidence: `src/services/scores.ts:167-190`; spec-8-4 explicitly recorded "scores.ts keeps it until 10.x revisits". Score/TTB DATA is fine for all users (shared rows + bump-all); only the per-user bookkeeping flags are wrong. Fix candidates: write the flags for ALL users (mirrors `updateSettingForAllUsers`), or move score-refresh state to a global/meta row.
  decision: 2026-07-17 re-homed HERE as the carrier (the code marker is corrected to point at this ledger). Gates nothing in epic 8 (single real user today); must land before a second real user relies on the scores banner.
  resolution: done 2026-07-19 (final deferred-work sweep) — the failure flag is ALL-USERS: `markScoresRefreshFailed`/`clearScoresRefreshFailed` write every user's row via new `setSettingForAllUsers` (INSERT…SELECT upsert over `user` — `updateSettingForAllUsers` only rewrites EXISTING rows, which would leave a row-less user's banner dark forever) and `deleteSettingForAllUsers`. The freshness stamp deliberately stays keyed to the driving (oldest) user: it is scheduler cadence bookkeeping, read back only by the scheduler that wrote it; score/TTB DATA was already shared rows. Pinned by "a provider throw persists the FR-40 failure flag for EVERY user" (second user with NO prior settings rows) in test/integration/scores.test.ts.

- source_spec: `_bmad-output/implementation-artifacts/spec-8-4-the-scheduled-refresh-serves-every-region-b4.md` (epic-8 follow-up review, 2026-07-17, LOW)
  summary: Migration 0017 leaves `ps_plus_region_state.last_user_activity` NULL for pre-8.4 rows, and the picker treats null activity as "brand-new — serve it", so a region whose only user went dormant before the deploy is refreshed every window forever.
  evidence: `migrations/0017_large_big_bertha.sql`; `src/services/psplus.ts` picker's idle-skip. Spend-only (no correctness harm), self-corrects on any future visit from that region's user. Not worth a backfill guess — leave until it costs something.
  resolution: discarded (accepted) 2026-07-19 final deferred-work sweep, Luca's call — the entry's own trigger ("until it costs something") has not fired: spend-only, self-correcting, and the only pre-8.4 region is Luca's own active one, so the dormant-region case cannot currently exist.

- source_spec: `_bmad-output/implementation-artifacts/spec-8-3-per-user-ps-facts-region-and-catalog-flag-b2-b3.md` (epic-8 follow-up review, 2026-07-17, LOW)
