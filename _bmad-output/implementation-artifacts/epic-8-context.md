...
- **Registration is OPEN (AD-29, signed off 2026-07-17)**: no allowlist, no admission table — "invite" is sharing the URL. Every door instead requires **proven control of the email**: magic link by construction, OAuth via `emailVerified`, and the account-LINK path links only on a **verified** email match (the account-takeover door). 8.2 deletes `isAllowedEmail`, the env var, the pre-gates, and the `requireAuth` re-check; no de-admission/ban tooling until abuse appears. Auth endpoints get WAF rate limits (never D1 counters) and a `verification`-row TTL sweep.
...
- **Read-budget fixes (8.6)**: single-row `WHERE id = ?` for game-by-id (currently loads the whole library), SQL `COUNT(*)` for settings counts, `LIMIT/OFFSET` paging for the catalog route, per-user library-version ETag for 304s on unchanged shelf refetches (every tracking writer bumps the version — a missed bump is a stale-304 bug), per-active-region version-keyed catalog caching (generation-keyed), and optionally better-auth `session.cookieCache` (TTL ≤5min). **Diff-based snapshot upserts were DROPPED at the 8.0 sign-off** (they strand unchanged rows on an old generation, breaking AD-28's sweep/prune — AD-33 §5).
- **Story 8.0 is DONE — AD-29..33 signed off by Luca 2026-07-17** (spine + memlog record the rulings). The gate is open: 8.2+ implement against AD-29 (open registration), AD-30 (per-region facts + departure ledger), AD-31 (region-state ledger cron), AD-32 (budget arithmetic incl. the 3–4-region/month rotation ceiling), AD-33 (paged delivery, ETag, no diff-upserts).
