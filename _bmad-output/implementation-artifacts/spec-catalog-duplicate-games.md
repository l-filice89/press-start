...
- [ ] post-deploy ops (manual, no code) -- run "Check PS+ Extra" once so the sweep upsert rewrites `title_normalized`; re-run the dup-pair probe query, expect suffix variants merged and grid deduped
