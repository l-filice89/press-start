...
2026-07-17 — independent pass RAN (fresh reviewer, no shared context) over 14ae6d2..HEAD, including an empirical `EXPLAIN QUERY PLAN` on the derivation SQL. **Region-wipe HIGH patch HOLDS** (`deleteCatalogOutsideRegion` gone, only scoped generation prune remains; region change writes only the setting; region rides in the shelf ETag; degenerate catalog hard-aborts before prune; two-region coexistence pinned). No new HIGHs. Findings addressed: **MED** missing `np_title_id` index made the np-leg EXISTS a per-row catalog walk (~10^5 rows on a full shelf, violating the spec's "all indexed probes" clause) → FIXED, index in schema + migration `0019`; **MED** the `leaving_on` legs read the SMALLEST `product_id` while the sweep writes the LARGEST, so a delisted sibling's NULL-dated row could permanently mask a live date → FIXED, all three legs now order dated rows first (`ORDER BY d.leaving_on IS NULL, d.product_id`); **LOW** schema doc contradicted DW-13 clear-not-delete semantics → comment fixed. Deferred with ledger entries: the leaving sweep's title-only target selection misses PSN_PRODUCT-linked (IGDB-retitled) games — a sweep-shape change; the lock-lost-mid-phase unstamped `left_on` (zero readers today). PRESERVE-VS-CLEAR rulings verified per endpoint (catalog `200 []` fail-closed; sweep absent-endTime = explicit-null write; failed query preserves). Obligation discharged; merge gate clear for this story.
