...
2026-07-17 — independent pass RAN (fresh reviewer, no shared context) over 14ae6d2..HEAD, tracing the per-region state machine end to end. Of the three HIGH patches: 304-path skip HOLDS, window poisoning HOLDS, backoff **PARTIALLY HELD** — the day gate keyed off `lastAttempt`, which `bad-region` outcomes never stamped, so a store-refused locale re-fired a REAL store fetch on every shelf GET (the original H2 scenario resurrected on the config-gap branch). **FIXED**: `recordRegionOutcome` gained an uncounted mode (`counted: false` — stamps the attempt, never feeds quarantine) and the guard now records it for `bad-region`/`no-region`; pinned by a route-level hazard test (one store fetch across two GETs, `failureCount` 0). Two further MEDs shared one root — `windowOf` was the calendar month while the cron window is days 15–28, so guard activity on days 1–14 could burn a quarantined region's window retry weeks early (~6-week blackout) or pre-satisfy cycle-complete and skip the mid-month Sony drop — **FIXED at the root**: `windowOf` now maps days 1–14 to the PREVIOUS month's window ([15th, next-15th) is the rotation unit), unit-pinned incl. the year boundary. **MED** window-opening fire ran a 2-statement reset per region beside a zero-headroom membership pass → **FIXED**: `resetStaleRegionWindows` resets all stale rows in ONE statement. Guard-path budget comment restated (~40-41 of 50, margin ~9). Deferred with ledger entry: 0017's NULL `last_user_activity` backfill (spend-only). Confirmed clean: guard cannot wedge or silently-stale a user, lock CAS + fences, mid-month regions, zero-user no-op, quarantine sort/skip arithmetic, cron overlap. Obligation discharged; merge gate clear for this story.
