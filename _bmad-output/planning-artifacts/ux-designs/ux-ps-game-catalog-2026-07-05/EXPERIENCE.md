...
**Installable PWA — one responsive app, three first-class surfaces.** The phone is where the "saw a game, want it" moment lives (a home-screen icon is the shortest path to it, FR-46); the desktop is where the cover-forward shelf shines. Not two builds — one responsive app with device-specific *deltas* (see Responsive & Platform).

- **Three destinations.** The **Shelf** is home; **Catalog** is the PS+ browse surface; **Stats** is the player record. Everything else surfaces *over* the active destination — modals, popovers, banners. Navigation is the URL (`/`, `/catalog`, `/stats`, `/game/:id`), owned by react-router; a destination is never a window event (ARCHITECTURE-SPINE AD-25).
...
Three destinations, switched by a **header toggle**; everything else hangs off whichever one is active. Search is the Shelf/Catalog action surface; Settings owns account, region, subscription, and backup actions. Anything needing attention appears in the **attention banner** under the header.
...
| **Stats** (`/stats`) | Header toggle | All-time cabinet recap plus a selected-year activity, milestone, and genre breakdown |
...
- **No FAB.** The one surviving chore, CSV export, lives in Settings as `DATA BACKUP / Keep your own copy`. The app mounts no hidden or empty launcher shell on Shelf, Catalog, or Stats.
- **Stats reading order.** On phone: title → all-time recap cards → native `CURRENT ROUND` select → selected-year round log and panels. Desktop keeps the same semantics while placing the select beside the title and the recap below.
...
→ Composition references: `wireframes/ia-shell-wireframe.html`, `mockups/card-flip-prototype.html`, `mockups/filter-row-wireframe.html`, `mockups/state-feedback-board.html`, `mockups/add-stragglers-flow.html`, `mockups/settings-login-mock.html`. These are historical composition references; their FAB, export, and handedness details are retired. Spine wins on conflict.
...
| **Filter row** | Groups: State (multiselect dropdown), Genre (multiselect dropdown), Time (multiselect dropdown, added 2026-07-16 Story 12.1: five half-open TTB hour bands ≤25/25–50/50–75/75–100/>100 + explicit Unknown, story/100% metric toggle pinned at the menu top — default story, filter state not a global setting; a null selected metric matches only Unknown), Flags (solid toggle pills), State-reveals (dashed pills — own group, amended 2026-07-10: an active reveal replaces the State group and shows only the matching hidden games; reveals OR among themselves, Genre/Flags still AND). OR within a group, AND across groups. A **live plain-English summary sentence** narrates the active filter so the model never needs decoding (OR-connectors in {colors.accent-glow}, AND-connectors in {colors.heat-magenta}). Active pills glow (FR-22). |
| **Settings backup** | Native button fetches `/api/export.csv`, downloads only a successful response as `press-start-library.csv`, shows `Exporting…` while pending, and toasts on failure without saving the error body. |
...
| **Catalog ordering** | **Alphabetical by title**, case-insensitive and locale-aware (`localeCompare`, `sensitivity: 'base'` — so `apex` and `Apex` sort together), matching the shelf's final tiebreaker exactly. **No state tier, no ownership tier** — those are the shelf's, and they answer *"what do I have that I can play?"*; the catalog answers *"what's in here?"*, where a predictable A–Z is how you scan ~490 games and find your way back to one. Games you already track are **not** hoisted, hidden, or dimmed: they sit in alphabetical order with their `In library` / `Owned` marker. **Reuse `compareShelf`'s title tiebreaker, not `compareShelf`** (`core/shelf.ts`) — reusing the card chrome does not mean reusing the sort. |
...
| **Region set, never refreshed** | `EMPTY CATALOG` while the automatic refresh path populates the region snapshot; no manual FAB action exists. |
...
- **Icon-only controls carry labels:** the owned toggle, flag icons, status pill, Settings control, and other icon-only buttons expose accessible names + state (e.g. "Owned, on"; "Playing — change status"). Status/milestone changes and toasts announce via a polite live region.
