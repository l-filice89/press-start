...
| **Catalog card** | Cover-forward, **shelf card chrome reused** — but it is *not* a shelf card: **no status pill, no owned toggle, no flip**, because a catalog game isn't a tracked game. Displays: cover, title, the `◈ PS+` membership flag, and **no release date** (the store payload has none). Three exclusive states: **already in library** (silver marker, no CTA — you can't add it twice), **addable** (`＋ Add` → add preview → on save, navigate to `/game/:id`), and, alongside Add, **`Claim now`** → opens the PS Store deep link in a new tab (leaves the app; nothing to confirm). |
...
- **Derived (never stored):** Released, Wishlisted (= **not owned AND not in the PS+ catalog** — amended 2026-07-14, AD-8: a game you can already play through the subscription is not a thing you still want to buy), Playable now (owned-or-PS+Extra AND released) — FR-12/13/14. "Anything that can be computed is computed."
- **A game added from the Catalog** is **not owned** (availability is not ownership) but **not wishlisted either** — it lands as `ownership_type: ps_plus`, playable now, on the shelf where it belongs.
...
6. **Climax:** the app lands on the game's detail — his now, playable now, not owned and *not* in his wishlist, because he isn't going to buy a game the subscription already gives him. The blind spot Epic 5 only *flagged* is now a place he can browse.
