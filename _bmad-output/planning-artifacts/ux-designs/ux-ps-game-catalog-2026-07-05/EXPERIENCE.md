...
| **Catalog ordering** | **Newest first** (the store's own `productReleaseDate` descending — free, and it's what a "what's new in the subscription" browse wants). **No ownership tier.** The shelf sorts owned-before-wishlisted because it answers *"what do I have that I can play?"*; the catalog answers *"what's out there I don't know about?"* — leading with games you already own would make a discovery surface open on the things you've already discovered. Tracked games are **not** hoisted and **not** hidden or dimmed: they sit where they fall, with their `In library` / `Owned` marker. **Do not reuse the shelf comparator** (`core/shelf.ts`) here — reusing the card chrome does not mean reusing its sort. |
